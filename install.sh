...
if [ $NON_INTERACTIVE -eq 0 ]; then
    ...
    if [[ "$input_cluster" =~ ^[Yy]$ ]]; then
        is_cluster="y"
        echo "--------------------------------------------------------------------------------"
        echo "IMPORTANT: Run this installation on a COMPUTE node, not the cluster head/login"
        echo "node. The generax and raxml-ng binaries are compiled with CPU/architecture-"
        echo "specific optimizations for the machine they are built on. If they are compiled"
        echo "on a head node whose architecture differs from the compute nodes, the binaries"
        echo "will crash when your jobs run. If the head and compute nodes share the same"
        echo "architecture this is not an issue, but building on a compute node (e.g. via an"
        echo "interactive job) is the safe choice."
        echo "--------------------------------------------------------------------------------"
