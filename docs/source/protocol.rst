...
.. tip::
  ...
  If you already have an alignment and want to skip straight to inferring the 
  trees and ancestors, you can use the command :code:`topiary-dataframe-from-fasta`
  to create a dataframe from your alignment. This will not have the information 
  necessary to reconcile the species and gene tree, so you will only be able to
  infer the gene tree and its ancestors. If you have done this step, you can skip
  to :ref:`infer-tree-and-anc-doc`.
...
.. code-block:: shell-session
  ...
  # Load the modules you used when compiling
  module load mpi/gcc/13.1.0/openmpi/4.1.6

  # Activate the topiary conda environment
  conda activate topiary

  topiary-alignment-to-ancestors final_dataframe.csv 
...
+ You need to make sure the relevant compiler and MPI modules are loaded. This 
  should match the modules you used when compiling topiary. (see :ref:`install-doc`)
+ You  need to make sure that the conda environment is active
  (:code:`conda activate topiary`).
...
The output for this call is a directory, *alignment-to-ancestors*. The primary 
output is *alignment-to-ancestors/results/index.html*. This file summarizes the 
...
installed. The *alignment-to-ancestors* directory will also have directories for each step
...
This step runs generax to generate a reconciled tree for each bootstrap replicate
generated by raxml-ng. If you have 1000 bootstrap replicates, this means generax
will be run 1000 times. Because each replicate is independent, this step is a
**re-entrant crawler**: you launch as many independent copies of
:code:`topiary-bootstrap-reconcile` as you like (for example one per compute node
via a SLURM job array), all pointing at the same run directory. They coordinate
through the shared filesystem — one builds the replicate directories, every copy
claims and runs replicates, and one assembles the final supports. There is no MPI
orchestration. Re-running the command resumes an interrupted calculation:
completed replicates are skipped and interrupted ones pick up from GeneRax's own
on-disk checkpoint.

By default each generax replicate runs on a single core, which is the most
throughput-efficient way to use a fixed core budget. If individual replicates are
too slow (very large trees), you can parallelize a single generax reconciliation
by passing a launcher prefix with :code:`--generax-launch`, e.g.
:code:`--generax-launch "mpirun -np 8"`. GeneRax parallelism is MPI-only, so this
requires an MPI-linked generax and a working single-node ``mpirun``; **you** are
responsible for making sure each crawler's node has the cores it asks for.
...
.. code-block:: shell-session
  ...
  # Each array task is an independent crawler. They coordinate through the
  # filesystem, so every task runs the exact same command.
  topiary-bootstrap-reconcile alignment-to-ancestors

The key aspects to note in this file are:

+ We launch 32 independent crawlers via a job array
  (:code:`#SBATCH --array=0-31`), each on its own node
  (:code:`#SBATCH --nodes=1 --ntasks=1`). Every array task runs the identical
  command; they self-organize through the run directory.
+ We give each crawler 8 cores (:code:`#SBATCH --cpus-per-task=8`). With the
  default (single-core generax) a crawler simply runs one replicate at a time;
  those cores are used only if you add :code:`--generax-launch "mpirun -np 8"`.
+ Because the crawler resumes automatically, a shorter per-task wall time with a
  job array is fine — if a task is preempted or times out, re-submitting picks up
  where it left off (interrupted replicates resume from their checkpoint). We
  allocated a day per task (:code:`#SBATCH --time=01-00:00:00`).
+ You need to make sure that the conda environment is active
  (:code:`conda activate topiary`). If you use :code:`--generax-launch` with
  ``mpirun``, also load the MPI module you compiled generax against.
...
When completed, this pipeline will update the *alignment-to-ancestors/results* directory, 
...
+ :code:`--generax-launch`. A launcher prefix (e.g. :code:`"mpirun -np 8"`)
  prepended to each generax command to parallelize an individual reconciliation.
  Empty (the default) runs each replicate single-core.

There is no :code:`--restart` flag — simply re-run the command (or re-submit the
job array) to resume an interrupted calculation.
